      ...
      <li class="popular-goods-item swiper-slide">
        ...
        <button type="button"
         class="category-btn${s===""?" active":""}"
          ...
          `}).join("");r.categoriesList.insertAdjacentHTML("beforeend",a)}let f=1;const q=8;let E=1;async function v(e,t,a=""){try{const s={limit:e,page:t};a&&(s.category=a);const o=(await p.get(`${M}furnitures`,{params:s})).data;E=Math.ceil(o.totalItems/Number(e)),t===1&&(r.furnitureGrid.innerHTML=""),X(allProducts),t>=E?ee():Z()}catch{l.error({title:"Помилка",message:"Не вдалося завантажити дані. Спробуйте пізніше",position:"topRight"})}}function X(e){const t=e.map(({_id:a,name:s,images:n,color:o,price:i})=>{const d=` <ul class="color-list"> 
        ...
        `}).join("");r.furnitureGrid.insertAdjacentHTML("beforeend",t),r.furnitureGrid.addEventListener("click",a=>{const s=a.target.closest(".furniture-btn");if(!s)return;const n=s.dataset.id,o=allProducts.find(i=>i._id===n);o?P([o]):l.error({title:"Error",message:"Продукт не знайдено за ID",position:"topRight"})})}function Y(e){if(!e.target.classList.contains("category-btn"))return;r.categoriesList.querySelectorAll(".category-btn").forEach(s=>s.classList.remove("active")),e.target.classList.add("active");const a=e.target.dataset.category;f=1,v(q,f,a)}function Z(){r.furnitureLoadMoreBtn.classList.remove("visually-hidden")}function ee(){r.furnitureLoadMoreBtn.classList.add("visually-hidden")}r.furnitureLoadMoreBtn.addEventListener("click",C);function C(e){f+=1;const t=r.categoriesList.querySelector(".category-btn.active"),a=t?t.dataset.category:"";v(q,f,a)}p.defaults.baseURL="https://furniture-store.b.goit.study/api";function te(){document.querySelector(".swiper").remove()}async function re(e=1){try{return(await p.get(`/feedbacks?limit=10&page=${e}`)).data.feedbacks}catch{te(),l.error({title:"Помилка",message:"Не вдалось завантажити дані. Спробуйте пізніше",position:"topRight"})}}async function oe(){const e=await re(),t=e.map(({descr:n,name:o,rate:i,_id:d})=>`
      <li class="feedback-item swiper-slide">
          <div id="rater-${d}" data-rating="${i}"></div>
          <p class="feedback-descr">${n}</p>
          <p class="feedback-name">${o}</p>
      </li>`).join("");r.feedbackList.insertAdjacentHTML("beforeend",t),se(e);const s=document.querySelector(".star-to-run").getAttribute("href");document.querySelectorAll(".star-value").forEach(n=>{n.style.backgroundImage=`url("${s}")`}),ae()}function se(e){e.forEach(({rate:t,_id:a})=>{w({max:5,readOnly:!0,rating:t,starSize:20,element:document.querySelector(`#rater-${a}`)})})}function ae(){new $(".feedback-swiper",{modules:[O,B],pagination:{el:".swiper-pagination",clickable:!0},navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"},grabCursor:!0,breakpoints:{375:{slidesPerView:1,spaceBetween:20},768:{slidesPerView:2,spaceBetween:24},1440:{slidesPerView:3,spaceBetween:24}}})}oe();
